...
_Bool vec_empty(const vec_t* vec);
...
void vec_pop(vec_t* vec) {
  ((vec->size) ? (void) (0) : __assert_fail ("vec->size", "data-structure/vector.c", 19, __PRETTY_FUNCTION__));
  --vec->size;
}
void vec_clear(vec_t* vec) {
  free(vec->data);
...
void* vec_at(vec_t* vec, int idx) {
  ((idx >= 0 && idx <= vec->size) ? (void) (0) : __assert_fail ("idx >= 0 && idx <= vec->size", "data-structure/vector.c", 30, __PRETTY_FUNCTION__));
  return vec->data + vec->width * idx;
}
void* vec_back(vec_t* vec) {
  ((vec->size) ? (void) (0) : __assert_fail ("vec->size", "data-structure/vector.c", 35, __PRETTY_FUNCTION__));
...
static void dijkstra(int x, node_t* vertices, int n) {
  int q_head = 0;
  vec_t q = (vec_t) { .size=0, .width=sizeof(int), .capacity=0, .data=((void *)0) };
  vertices[x].dist = 0;
  vec_push(&q, &x);
  while (q_head < q.size) {
...
int main() {
  ...
  for (int i = 0; i < n; ++i) {
    vertices[i].dist = 0x7fffffff;
    vertices[i].kids = (vec_t) { .size=0, .width=sizeof(int), .capacity=0, .data=((void *)0) };
    vertices[i].parents = (vec_t) { .size=0, .width=sizeof(int), .capacity=0, .data=((void *)0) };
  }
  while (1) {
    int src, des;
  ...
  print_all_shortest_path(5, &(vec_t) { .size=0, .width=sizeof(int), .capacity=0, .data=((void *)0) }, vertices, n);
  return 0;
}
